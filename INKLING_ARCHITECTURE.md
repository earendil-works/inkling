...
Preview rendering is debounced and must not block typing or collaboration. It parses publication frontmatter, omits it from rendered prose, and reflects valid draft metadata immediately without changing authorization. Author email identifiers display the corresponding workspace account name when the people directory knows it and otherwise display the email address. The source and preview panes synchronize scrolling on a best-effort basis in either direction. Preview block elements carry Markdown source ranges. Synchronization samples source lines in three-line intervals and linearly interpolates their rendered offsets, using an anchor about three line heights below each viewport's top so corresponding content retains readable context without jumping. An explicit zero-to-zero mapping keeps both panes at the true document top and smoothly interpolates through preview-only headers before the first rendered Markdown block. Mermaid diagram rendering is separately debounced and preserves pan and zoom state for unchanged diagrams where practical.
