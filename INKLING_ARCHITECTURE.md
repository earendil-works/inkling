...
Preview rendering is debounced and must not block typing or collaboration. It parses publication frontmatter, omits it from rendered prose, and reflects valid draft metadata immediately without changing authorization. Author email identifiers display the corresponding workspace account name when the people directory knows it and otherwise display the email address. The source and preview panes synchronize scrolling on a best-effort basis in either direction. Preview block elements carry Markdown source ranges; synchronization maps the source position at one viewport's midpoint to the corresponding rendered block position at the other viewport's midpoint rather than matching scroll percentages. Mermaid diagram rendering is separately debounced and preserves pan and zoom state for unchanged diagrams where practical.
